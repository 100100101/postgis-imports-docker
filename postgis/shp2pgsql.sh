#!/bin/bash
source ../.env
PATH_TO_SHAPE=$CONT_SHAPES_PATH/$SHP_IMPORT_FILENAME
echo '---'
echo $CONT_SHAPES_PATH
echo $SHP_IMPORT_FILENAME
echo $SHP_IMPORT_TABLE_NAME
echo '---'


docker exec $POSTGIS_CONTAINER_NAME bash -c "shp2pgsql -s $PROJECTION_SRID $PATH_TO_SHAPE $SHP_IMPORT_TABLE_NAME | PGPASSWORD=$POSTGRES_PASSWORD psql -d $POSTGRES_DB -U $POSTGRES_USER -h $POSTGRES_HOST -p $INNER_POSTGRES_PORT"

# PGPASSWORD=pass psql -d gis -U gisuser -h 172.17.0.1 -p 5432
