...
PATH_TO_SHAPE=$CONT_SHAPES_PATH/$SHP_FILENAME
...
docker exec $POSTGIS_CONTAINER_NAME bash -c "shp2pgsql -s $PROJECTION_SRID $PATH_TO_SHAPE $SHP_IMPORT_FILENAME | PGPASSWORD=$POSTGRES_PASSWORD psql -d $POSTGRES_DB -U $POSTGRES_USER -h $POSTGRES_HOST -p $INNER_POSTGRES_PORT"

# docker exec $POSTGIS_CONTAINER_NAME bash -c "echo $PROJECTION_SRID"
