...
# Если в системе недостаточно оперативной памяти, вы можете добавить параметры -s, --slim и --drop. В этом случае используется меньше основной памяти, вместо этого в базе данных хранится больше данных. Однако это значительно замедляет импорт. Подробнее об этих параметрах см. главу Средний .